...
const LessonView: React.FC<LessonViewProps> = ({ lesson, onComplete, onBack }) => {
  ...
  const fetchLessonContent = useCallback(async () => {
    try {
      ...
      setError('No se pudo generar el contenido. Verifica la configuración de tu clave de API y tu conexión a internet, y vuelve a intentarlo.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  if (error || !content) {
    return (
      <div className="text-center text-red-400 p-8 bg-brand-dark-light rounded-lg">
        <p className="text-lg">{error}</p>
        <button onClick={onBack} className="mt-6 px-6 py-2 bg-accent text-white rounded-lg hover:bg-sky-500 transition-colors">Volver</button>
      </div>
    );
  }
