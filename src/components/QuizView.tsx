...
const QuizView: React.FC<QuizViewProps> = ({ quiz, onComplete, onBack }) => {
  ...
  const fetchQuizData = useCallback(async () => {
    try {
      ...
      setError('No se pudo generar el cuestionario. Verifica la configuración de tu clave de API y tu conexión a internet, y vuelve a intentarlo.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  if (error || !quizData) {
    return <div className="text-center text-red-400 p-8 bg-brand-dark-light rounded-lg"><p className="text-lg">{error}</p><button onClick={onBack} className="mt-6 px-6 py-2 bg-accent text-white rounded-lg hover:bg-sky-500 transition-colors">Volver</button></div>;
  }
    ...
    return (
      <div className="max-w-2xl mx-auto text-center p-8 bg-brand-dark-light rounded-lg shadow-xl animate-fade-in">
        <div className="flex items-center justify-center gap-3">
             <Icon name={passed ? 'checkCircle' : 'xCircle'} className={`w-8 h-8 ${passed ? 'text-brand-primary' : 'text-red-500'}`} />
            <h2 className="text-3xl font-bold text-white">Resultados del Cuestionario</h2>
        </div>
